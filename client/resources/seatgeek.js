...
const getEventsFromSeatGeek = async (lat, lon, maxRadius) => {
    try {
        const key = SEAT_GEEK_KEY;
        const { data } = await axios.get(`https://api.seatgeek.com/2/events?lat=${lat}&lon=${lon}&range=${maxRadius}mi&per_page=10&client_id=${key}`);
        // console.log('EVENTS:', events);
        return data.events;
    } catch (err) {
        console.log('error: ', err);
